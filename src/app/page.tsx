      ...
      <div className="flex flex-col h-screen bg-background">
        <header className="flex h-16 shrink-0 items-center gap-4 border-b px-4 md:px-6 bg-background">
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-lg font-headline">My Drafts</h1>
            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Local Workspace</p>
          </div>
        </header>
          ...
          <div className="mx-auto max-w-7xl space-y-6">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center py-24 border-2 border-dashed rounded-xl bg-card">
                ...
                <Button variant="outline" className="mt-4" onClick={() => router.push('/new-order')}>
                  ...
                  <TableBody>
                    {filteredDrafts.map((draft) => (
                      <TableRow 
                        ...
                        onClick={() => { 
                          loadDraft(draft); 
                          // Handle legacy Drafts that might have '/' as currentStep
                          const targetStep = (!draft.currentStep || draft.currentStep === '/') ? '/new-order' : draft.currentStep;
                      ...
                      >
                        ...
                        <TableCell className="text-muted-foreground text-xs font-medium">
                          {formatDate(draft.eventDetails?.eventDate)}
                        </TableCell>
                        <TableCell className="text-muted-foreground text-xs font-medium">
